 ...
 * @since  11.1
 */
class JFormFieldFaselector extends JFormField
{
	/**
	 * The form field type.
	 *
	...
	protected function getInput()
	{
		...
		$class        = !empty($this->class) ? ' class="' . $this->class . '"' : ' class="faicon"';
		...
		$document->addStyleSheet(JURI::root().'media/mod_txcustom/css/icon-selector.css');
		$document->addScript(JURI::root().'media/mod_txcustom/js/icon-selector.js');
		...
		$html[] = '<input class="input-xs input-mini" size="4" type="text" placeholder="Size" name="' . $this->name . '[size]"' . $dirname . $list
			. ' value="'
			. htmlspecialchars($size, ENT_COMPAT, 'UTF-8') . '"'
			. ' />' ;
		$html[] = $datalist;
